...
`ComfyUI-LLM Agent SDK` discovers the workflows under `skills/`, selects one
...
The node returns an `IMAGE` preview batch, text summary, local output path,
artifact JSON, and execution state JSON. Full-resolution originals are kept in
the returned output directory. `skill_override=auto` is the normal mode; select
a skill explicitly only to correct or debug routing.
...
requires it. In that case `text` contains the deliverable, `artifacts_json` is
an empty list, `state_json.delivery_mode` is `text`, and `IMAGE` is only an
empty preview placeholder for ComfyUI type compatibility.
